      ...
      if(guard){ // this is a complex object
        ...
        validate(input, function(err){
          ...
          keys.forEach(function(_key){
            if(!input){
              if(opts.required) errors.push({ key: _key, error: opts.required === true ? 'Required property not provided.' : opts.required });
        ...
        if(opts.required && !input && input !== false && !opts.default){
          ...
          cb(opts.required === true ? 'Required property not provided.' : opts.required);
        }else if(opts.default && !input && input !== false){
          // supply default value if nothing else passed in
          var d = opts.default;
          if(typeof d === 'function') d = d();
          cb(null, d);
        }else if(input || input === false){
          var sanitize = opts.sanitize || function(_input, _cb){
            _cb(null, _input); // pass-through sanitization
          };
          ...
          // sanitize value if sanitization function was specified
          sanitize(input, function(err){
